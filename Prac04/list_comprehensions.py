...
numbers = [number for number in almost_numbers if number >= 0]
print(numbers)
...
GREATER_NUMBER = 9
greater_numbers = [number for number in numbers if number > GREATER_NUMBER]
...
CHECKING_LENGTH = 11
long_names = [full_name for full_name in full_names if len(full_name) > CHECKING_LENGTH]
last_name = ' '.join(long_names).split()
print(last_name[1:6:2])
